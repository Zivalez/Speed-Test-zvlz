	...
	(function() {
		...
		t.prototype = {
			...
			_setupCodecs: function() {
				...
				var r = t.canPlayType("audio/mpeg;").replace(/^no$/, ""), i = e._navigator ? e._navigator.userAgent : "", a = i.match(/OPR\/([0-6].)/g), o = a && parseInt(a[0].split("/")[1], 10) < 33, s = i.indexOf("Safari") !== -1 && i.indexOf("Chrome") === -1, c = i.match(/Version\/(.*?) /), l = s && c && parseInt(c[1], 10) < 15;
				return e._codecs = {
		...
		r.prototype = {
			init: function(e) {
				var t = this;
				return n.ctx || u(), t._autoplay = e.autoplay || !1, t._format = typeof e.format == "string" ? [e.format] : e.format, t._html5 = e.html5 || !1, t._muted = e.mute || !1, t._loop = e.loop || !1, t._pool = e.pool || 5, t._preload = typeof e.preload != "boolean" && e.preload !== "metadata" || e.preload, t._rate = e.rate || 1, t._sprite = e.sprite || {}, t._src = typeof e.src == "string" ? [e.src] : e.src, t._volume = e.volume === void 0 ? 1 : e.volume, t._xhr = {
			...
			load: function() {
				...
				return t ? (e._src = t, e._state = "loading", window.location.protocol === "https:" && t.slice(0, 5) === "http:" && (e._html5 = !0, e._webAudio = !1), new i(e), e._webAudio && o(e), e) : void e._emit("loaderror", null, "No codec support for selected audio sources.");
			},
			play: function(e, t) {
				var r = this, i = null;
				if (typeof e == "number") i = e, e = null;
				else {
					if (typeof e == "string" && r._state === "loaded" && !r._sprite[e]) return null;
					if (e === void 0 && (e = "__default", !r._playLock)) {
				...
				var l = Math.max(0, s._seek > 0 ? s._seek : r._sprite[e][0] / 1e3), u = Math.max(0, (r._sprite[e][0] + r._sprite[e][1]) / 1e3 - l), d = 1e3 * u / Math.abs(s._rate), f = r._sprite[e][0] / 1e3, p = (r._sprite[e][0] + r._sprite[e][1]) / 1e3;
				s._sprite = e, s._ended = !1;
				var m = function() {
					s._paused = !1, s._seek = l, s._start = f, s._stop = p, s._loop = !(!s._loop && !r._sprite[e][2]);
				};
				if (l >= p) return void r._ended(s);
				var h = s._node;
				if (r._webAudio) {
					var g = function() {
					...
					var _ = function() {
						...
						try {
							...
							})) : t || (r._playLock = !1, m(), r._emit("play", s._id)), h.playbackRate = s._rate, h.paused) return void r._emit("playerror", s._id, "Playback was unable to start. This is most commonly an issue on mobile devices and Chrome where playback was not within a user interaction.");
			...
			mute: function(e, t) {
				...
				if (t === void 0) {
					if (typeof e != "boolean") return r._muted;
					r._muted = e;
				}
				for (var i = r._getSoundIds(t), a = 0; a < i.length; a++) {
					var o = r._soundById(i[a]);
					o && (o._muted = e, o._interval && r._stopFade(o._id), r._webAudio && o._node ? o._node.gain.setValueAtTime(e ? 0 : o._volume, n.ctx.currentTime) : o._node && (o._node.muted = !!n._muted || e), r._emit("mute", o._id));
				}
				return r;
			},
			volume: function() {
				...
				i.length === 1 || i.length === 2 && i[1] === void 0 ? r._getSoundIds().indexOf(i[0]) >= 0 ? t = parseInt(i[0], 10) : e = parseFloat(i[0]) : i.length >= 2 && (e = parseFloat(i[0]), t = parseInt(i[1], 10));
				var a;
				if (!(e !== void 0 && e >= 0 && e <= 1)) return a = t ? r._soundById(t) : r._sounds[0], a ? a._volume : 0;
				if (r._state !== "loaded" || r._playLock) return r._queue.push({
					event: "volume",
					action: function() {
						r.volume.apply(r, i);
				...
				t === void 0 && (r._volume = e), t = r._getSoundIds(t);
				for (var o = 0; o < t.length; o++) (a = r._soundById(t[o])) && (a._volume = e, i[2] || r._stopFade(t[o]), r._webAudio && a._node && !a._muted ? a._node.gain.setValueAtTime(e, n.ctx.currentTime) : a._node && !a._muted && (a._node.volume = e * n.volume()), r._emit("volume", a._id));
				return r;
			},
			...
			_startFadeInterval: function(e, t, n, r, i, a) {
				...
				e._fadeTo = n, e._interval = setInterval(function() {
					var i = (Date.now() - d) / r;
					d = Date.now(), s += c * i, s = Math.round(100 * s) / 100, s = c < 0 ? Math.max(n, s) : Math.min(n, s), o._webAudio ? e._volume = s : o.volume(s, e._id, !0), a && (o._volume = s), (n < t && s <= n || n > t && s >= n) && (clearInterval(e._interval), e._interval = null, e._fadeTo = null, o.volume(n, e._id), o._emit("fade", e._id));
				}, u);
			},
			...
			loop: function() {
				...
				if (i.length === 1) {
					if (typeof i[0] != "boolean") return !!(n = r._soundById(parseInt(i[0], 10))) && n._loop;
					e = i[0], r._loop = e;
				} else i.length === 2 && (e = i[0], t = parseInt(i[1], 10));
				for (var a = r._getSoundIds(t), o = 0; o < a.length; o++) (n = r._soundById(a[o])) && (n._loop = e, r._webAudio && n._node && n._node.bufferSource && (n._node.bufferSource.loop = e, e && (n._node.bufferSource.loopStart = n._start || 0, n._node.bufferSource.loopEnd = n._stop, r.playing(a[o]) && (r.pause(a[o], !0), r.play(a[o], !0)))));
				return r;
			},
			rate: function() {
				var e, t, r = this, i = arguments;
				i.length === 0 ? t = r._sounds[0]._id : i.length === 1 ? r._getSoundIds().indexOf(i[0]) >= 0 ? t = parseInt(i[0], 10) : e = parseFloat(i[0]) : i.length === 2 && (e = parseFloat(i[0]), t = parseInt(i[1], 10));
				var a;
				if (typeof e != "number") return a = r._soundById(t), a ? a._rate : r._rate;
				if (r._state !== "loaded" || r._playLock) return r._queue.push({
					event: "rate",
					action: function() {
						r.rate.apply(r, i);
				...
				t === void 0 && (r._rate = e), t = r._getSoundIds(t);
				for (var o = 0; o < t.length; o++) if (a = r._soundById(t[o])) {
					r.playing(t[o]) && (a._rateSeek = r.seek(t[o]), a._playStart = r._webAudio ? n.ctx.currentTime : a._playStart), a._rate = e, r._webAudio && a._node && a._node.bufferSource ? a._node.bufferSource.playbackRate.setValueAtTime(e, n.ctx.currentTime) : a._node && (a._node.playbackRate = e);
					var s = r.seek(t[o]), c = 1e3 * ((r._sprite[a._sprite][0] + r._sprite[a._sprite][1]) / 1e3 - s) / Math.abs(a._rate);
					!r._endTimers[t[o]] && a._paused || (r._clearTimer(t[o]), r._endTimers[t[o]] = setTimeout(r._ended.bind(r, a), c)), r._emit("rate", a._id);
				}
				return r;
			},
			seek: function() {
				var e, t, r = this, i = arguments;
				if (i.length === 0 ? r._sounds.length && (t = r._sounds[0]._id) : i.length === 1 ? r._getSoundIds().indexOf(i[0]) >= 0 ? t = parseInt(i[0], 10) : r._sounds.length && (t = r._sounds[0]._id, e = parseFloat(i[0])) : i.length === 2 && (e = parseFloat(i[0]), t = parseInt(i[1], 10)), t === void 0) return 0;
				if (typeof e == "number" && (r._state !== "loaded" || r._playLock)) return r._queue.push({
					event: "seek",
					action: function() {
						r.seek.apply(r, i);
					}
				...
				if (a) {
					if (!(typeof e == "number" && e >= 0)) {
						if (r._webAudio) {
							var o = r.playing(t) ? n.ctx.currentTime - a._playStart : 0, s = a._rateSeek ? a._rateSeek - a._seek : 0;
							return a._seek + (s + o * Math.abs(a._rate));
						}
						return a._node.currentTime;
					}
					var c = r.playing(t);
					c && r.pause(t, !0), a._seek = e, a._ended = !1, r._clearTimer(t), r._webAudio || !a._node || isNaN(a._node.duration) || (a._node.currentTime = e);
					var l = function() {
						c && r.play(t, !0), r._emit("seek", t);
					};
					if (c && !r._webAudio) {
						var u = function() {
							r._playLock ? setTimeout(u, 0) : l();
						};
						setTimeout(u, 0);
					} else l();
				}
		...
		i.prototype = {
			...
			create: function() {
				...
				return t._webAudio ? (e._node = n.ctx.createGain === void 0 ? n.ctx.createGainNode() : n.ctx.createGain(), e._node.gain.setValueAtTime(r, n.ctx.currentTime), e._node.paused = !0, e._node.connect(n.masterGain)) : n.noAudio || (e._node = n._obtainHtml5Audio(), e._errorFn = e._errorListener.bind(e), e._node.addEventListener("error", e._errorFn, !1), e._loadFn = e._loadListener.bind(e), e._node.addEventListener(n._canPlayEvent, e._loadFn, !1), e._endFn = e._endListener.bind(e), e._node.addEventListener("ended", e._endFn, !1), e._node.src = t._src, e._node.preload = !0 === t._preload ? "auto" : t._preload, e._node.volume = r * n.volume(), e._node.load()), e;
			},
			reset: function() {
				var e = this, t = e._parent;
			...
			_loadListener: function() {
				var e = this, t = e._parent;
				t._duration = Math.ceil(10 * e._node.duration) / 10, Object.keys(t._sprite).length === 0 && (t._sprite = { __default: [0, 1e3 * t._duration] }), t._state !== "loaded" && (t._state = "loaded", t._emit("load"), t._loadQueue()), e._node.removeEventListener(n._canPlayEvent, e._loadFn, !1);
			},
			_endListener: function() {
				var e = this, t = e._parent;
				t._duration === Infinity && (t._duration = Math.ceil(10 * e._node.duration) / 10, t._sprite.__default[1] === Infinity && (t._sprite.__default[1] = 1e3 * t._duration), t._ended(e)), e._node.removeEventListener("ended", e._endFn, !1);
			}
		};
		var a = {}, o = function(e) {
			var t = e._src;
			...
			if (/^data:[^;]+;base64,/.test(t)) {
				...
				o.open(e._xhr.method, t, !0), o.withCredentials = e._xhr.withCredentials, o.responseType = "arraybuffer", e._xhr.headers && Object.keys(e._xhr.headers).forEach(function(t) {
					...
					var t = (o.status + "")[0];
					if (t !== "0" && t !== "2" && t !== "3") return void e._emit("loaderror", null, "Failed loading audio file with status: " + o.status + ".");
					c(o.response, e);
				}, o.onerror = function() {
