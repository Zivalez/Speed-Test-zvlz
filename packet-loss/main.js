...
var Packet = class {
  ...
  spawnParticles(x, y, color, count = 8) {
    // The persistent packet color/state already communicates loss or success.
    return;
  }
};
var VisualizationEngine = class {
  ...
  renderStats(layout) {
    ...
    const statsConfig = [
      { id: "sent", label: "Sent", color: "#e84d2a", bg: "rgba(232, 77, 42, 0.1)" },
      { id: "loss", label: "Total Loss", color: "#9f3045", bg: "rgba(159, 48, 69, 0.1)" },
      { id: "lost", label: "Lost", color: "#531d2a", bg: "rgba(83, 29, 42, 0.1)" }
    ];
  ...
  updateNetworkLabels() {
    ...
    if (!rttLabel) {
      ...
      rttLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (!intervalLabel) {
      ...
      intervalLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (!jitterLabel) {
      ...
      jitterLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (!sentPPSLabel) {
      ...
      sentPPSLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (!receivedPPSLabel) {
      ...
      receivedPPSLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (!outOfOrderLabel) {
      ...
      outOfOrderLabel.setAttribute("fill", "var(--zvlz-muted)");
    ...
    if (this.running && this.showNetworkMetrics) {
      ...
      if (outOfOrderValue >= 5) {
        outOfOrderLabel.setAttribute("fill", "#531d2a");
        outOfOrderLabel.setAttribute("opacity", "0.9");
      } else if (outOfOrderValue >= 1) {
        outOfOrderLabel.setAttribute("fill", "#e84d2a");
  ...
  updateDirectionalLossLabels(c2sLoss, s2cLoss) {
    ...
    if (!c2sLabel) {
      ...
      c2sLabel.setAttribute("fill", "#e84d2a");
    ...
    if (this.direction === "both") {
      ...
      c2sLabel.setAttribute("fill", displayC2S > 20 ? "#531d2a" : "#e84d2a");
    } else {
    ...
    if (!s2cLabel) {
      ...
      s2cLabel.setAttribute("fill", "#9f3045");
    ...
    if (this.direction === "both") {
      ...
      s2cLabel.setAttribute("fill", displayS2C > 20 ? "#531d2a" : "#9f3045");
    } else {
...
testEngine.on("test:error", (data) => {
  document.dispatchEvent(new CustomEvent("zvlz:packet-error", { detail: data }));
...
function handleResultsRedirect(stats) {
  // Results remain in the self-hosted ZVLZ interface.
}
testEngine.on("test:completed", (stats) => {
  isTestFinished = true;
  lastTestStats = stats;
  document.dispatchEvent(new CustomEvent("zvlz:packet-completed", { detail: stats }));
...
function initializePlatformFeatures() {
  // The ZVLZ self-hosted build keeps completed results on this page.
}
initializeTheme();
restoreSettings();
